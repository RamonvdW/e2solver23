...
class Command(BaseCommand):
    ...
    help = "Load Ring3 into a progress"
    ...
    def add_arguments(self, parser):
        parser.add_argument('nr', type=int, help='Ring3 to load')
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write('[INFO] Loading Ring3 %s into processor %s' % (nr, self.processor))

        try:
            ring3 = Ring3.objects.get(nr=nr)
        except Ring3.DoesNotExist:
            self.stderr.write('[ERROR] Ring3 not found')
            return
        ...
        for loc in locs:
            ...
            field_str = 'loc%s' % loc
            p2x2_nr = getattr(ring3, field_str)
            # print('loc %s ring3 %s = %s' % (loc, field_str, p2x2_nr))
            if p2x2_nr > 0:
