...
class Command(BaseCommand):
    ...
    def _select_next_loc(self):
        ...
        self.stdout.write('[DEBUG] select_next_loc failed %s' % msg)
        return -1, True
    ...
    def _find_reduce(self, segment):
        ...
        for side in sides:
            ...
            for p in self._iter(loc, options_side1, options_side2, options_side3, options_side4):
                ...
                self.deadline = time.monotonic() + 60 * 30      # 30 minutes
    ...
    def handle(self, *args, **options):
        ...
        if side == 1:
            self.locs = (1, 2, 3, 4, 10, 9, 11, 5, 6, 7, 8, 15, 16, 14, 13, 12, 11, 17, 18, 19, 20, 21, 22, 23, 24)
            segments = (130, 131, 132, 133, 134, 135, 136, 9, 10, 11, 12, 13, 14, 15, 16,
                        138, 139, 140, 141, 142, 143, 144, 17, 18, 19, 20, 21, 22, 23, 24)
        elif side == 2:
            self.locs = (8, 16, 24, 32, 15, 7, 23, 40, 48, 56, 64, 55, 63, 47, 39, 31, 6, 14, 22, 30, 38, 46, 54, 62)
            segments = (16, 24, 32, 40, 48, 56, 64, 136, 144, 152, 160, 168, 176, 184, 192,
                        15, 23, 31, 39, 47, 55, 63, 135, 143, 151, 159, 167, 175, 183, 191)
        elif side == 3:
            self.locs = (57, 58, 59, 60, 50, 49, 51, 61, 62, 63, 64, 55, 56, 54, 53, 52, 41, 42, 43, 44, 45, 46, 47, 48)
            segments = (186, 187, 188, 189, 190, 191, 192, 57, 58, 59, 60, 61, 62, 63, 64,
                        178, 179, 180, 181, 182, 183, 184, 49, 50, 51, 52, 53, 54, 55, 56)
        else:
            self.locs = (1, 9, 17, 25, 10, 2, 18, 33, 41, 49, 57, 50, 58, 42, 34, 26, 3, 11, 19, 27, 35, 43, 51, 59)
            segments = (9, 17, 25, 33, 41, 49, 57, 130, 138, 146, 154, 162, 170, 178, 186,
                        10, 18, 26, 34, 42, 50, 58, 131, 139, 147, 155, 163, 171, 179, 187)
